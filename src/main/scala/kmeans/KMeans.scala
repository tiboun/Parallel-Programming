...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val grouped: GenMap[Point, GenSeq[Point]] = points.groupBy(p => findClosest(p, means))
    means.filter(m => grouped.get(m) match {
      case Some(_) => false
      case _ => true
    }).foldLeft(grouped)((genMap, point) => genMap.updated(point, List()))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val convergeResult = for (
      i <- 0 until oldMeans.size
    ) yield {
      oldMeans(i).squareDistance(newMeans(i)) <= eta
    }
    !convergeResult.exists(_ == false)
  }
