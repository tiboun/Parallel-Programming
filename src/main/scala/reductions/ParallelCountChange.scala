...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    if (money == 0) 1
      ...
      val (lRes, rRes) = (countChange(money - coins.head, coins), countChange(money, coins.tail))
      lRes + rRes
    }
  }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (money == 0) 1
      ...
      val (lRes, rRes) = common.parallel(parCountChange(money - coins.head, coins, threshold), parCountChange(money, coins.tail, threshold))
      lRes + rRes
    }
  }
